...
export const Navbar = ({ id, editor, activeTool, onChangeActiveTool }: NavbarProps) => {
  ...
  const exportVariants = [
    {
      name: 'JSON',
      desc: 'Save for later editing',
      icon: <FileJson />,
      onClick: () => editor?.saveJson(),
    },
    {
      name: 'PNG',
      desc: 'Best for sharing on the web',
      icon: <FileImage />,
    ...
    {
      name: 'SVG',
      desc: 'Best for editing in vector software',
      icon: <FileImage />,
  ...
  return (
    <nav className='w-full flex items-center p-4 h-[68px] gap-x-8 border-b lg:pl-[34px]'>
      ...
      <div className='w-full flex items-center gap-x-1 h-full'>
        ...
        {!isPending && !isError && (
          <div className='flex items-center gap-x-2'>
            <BadgeCheck className='size-[20px] text-muted-foreground' />
        ...
        <div className='ml-auto flex items-center gap-x-4'>
          <DropdownMenu modal={false}>
            <DropdownMenuTrigger asChild>
              <Button size='sm' variant='ghost'>
                Export
                <Download className='size-4 ml-3' />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align='end' className='min-w-60'>
              {exportVariants.map((item, inx) => (
                <DropdownMenuItem
                  key={inx}
                  className='flex items-center gap-x-2 [&_svg]:text-neutral-500'
